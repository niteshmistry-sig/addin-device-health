...
// If MyGeotab calls initialize() with the real API, _hostCalled becomes true
// and mock data is skipped. Otherwise, after a delay, mock data is used for preview.
(function () {
    if (typeof document === "undefined") { return; }

    var _hostCalled = false;
    var _origInit = geotab.addin.device_health_diagnostics.initialize;
    geotab.addin.device_health_diagnostics.initialize = function (api, page, cb) {
        _hostCalled = true;
        return _origInit(api, page, cb);
    };
    function tryMockInit() {
        if (!_hostCalled) {
            geotab.addin.device_health_diagnostics._initStandalone();
        }
    }
    if (document.readyState === "loading") {
        document.addEventListener("DOMContentLoaded", function () { setTimeout(tryMockInit, 2000); });
    } else {
        setTimeout(tryMockInit, 2000);
    }
})();
