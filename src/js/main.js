...
geotab.addin.device_health_diagnostics = function () {
    "use strict";
...
// When running inside MyGeotab, the host calls the factory function
// geotab.addin.device_health_diagnostics() and then calls initialize(api, state, cb).
// In standalone mode (opening the HTML directly), we invoke the factory ourselves
// and use mock data for preview.
(function () {
    if (typeof document === "undefined") { return; }

    // If we're running standalone (not inside MyGeotab), the factory won't be called
    // by any host. Wait briefly, then self-initialize with mock data.
    function tryStandaloneInit() {
        // Check if the factory is still a function (MyGeotab hasn't called it yet)
        if (typeof geotab.addin.device_health_diagnostics === "function") {
            // Factory not yet invoked by MyGeotab — we're standalone.
            // Invoke it ourselves and use mock data.
            var addin = geotab.addin.device_health_diagnostics();
